...
type Props = NativeStackScreenProps<AppStackParamList, "EditStudent">;
...
export function EditStudentScreen({ navigation, route }: Props) {
  const { collegeId, student } = route.params;
  ...
  const [dob, setDob] = useState(isoToYmd(student.dob));
  const [address, setAddress] = useState(student.address);
  const [mobile, setMobile] = useState(student.mobile);
  const [busId, setBusId] = useState<string | null>(student.bus?._id ?? null);
  const [stop, setStop] = useState<string | null>(student.stop ?? null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([b, s]) => {
        ...
        for (const st of s) {
          if (st._id === student._id) continue;
  ...
  async function onSubmit() {
    ...
    setBusy(true);
    try {
      await collegeStudentsApi.update(collegeId, student._id, {
        name: name.trim(),
    ...
    <View style={styles.root}>
      <View style={styles.appBar}>
        ...
        <View style={{ flex: 1, alignItems: "center" }}>
          <Text style={styles.appTitle}>Edit Student</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        ...
        >
          ...
          <Text style={styles.groupLabel}>Bus assignment</Text>
            ...
            <View style={styles.busList}>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.busOptionSub}>Remove from any bus</Text>
                </View>
...
function makeStyles(colors: Colors) {
  return StyleSheet.create({
    ...
    seatPillText: { color: colors.text, fontSize: 11, fontWeight: "800" },
    seatPillTextFull: { color: colors.danger },

    tick: { color: colors.accent, fontSize: 18, fontWeight: "800" },
